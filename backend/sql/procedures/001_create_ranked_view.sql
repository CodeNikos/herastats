-- procedure: create_ranked_view(IN tour_id integer, IN div text, IN grupo text)
-- Extraído automáticamente. Aplicar con: npm run db:procedures
CREATE OR REPLACE PROCEDURE public.create_ranked_view(IN tour_id integer, IN div text, IN grupo text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
  view_name text;
BEGIN
  ...
  view_name := format('ranked_%s_%s_%s', tour_id, div, grupo);
